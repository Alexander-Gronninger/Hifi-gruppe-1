...
export async function lazyLoadProducts(start, offset) {
  let response = await fetch(`${API_URL}?_start=${start}&_end=${offset}`);
  let json = await response.json();
  console.log(json);
  localDatabase = json;
  ...
  json.forEach(function (data, index) {
      ...
      productMainGrid__element.appendChild(article)

      article.innerHTML = `<a class="product__compareBtn" href=""
      >Compare
      <img
    ...
    <div class="product__cartStockContainer">
      <button class="cartStockContainer__btn g-button btn">Add to cart</button>
      ...
      let stockIcons = article.querySelector(".availability__icon");
      if (data.stock < 2) {
      ...
      article.addEventListener("click", function (event) {
        ...
        if (event.target.classList.contains("cartStockContainer__btn")){
          localStorage.setItem("cart", "lort")
        } else{
      ...
      if (index === 3) {
          const intersectionObserver = new IntersectionObserver(function (entries) {
              ...
              lazyLoadProducts(start + 4, offset + 4)
          }
          )
