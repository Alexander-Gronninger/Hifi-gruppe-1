...
function sendMessage(message) {
    ...
    if (message.toLowerCase().includesOneOf(swearWords)) {
    ...
    //if msg contains hey
    if (
        message.toLowerCase().includesOneOf(["hey", "hej", "hi", "hello", "yo"]) &&
        message.length < 16
    ) {
        pushMessage("left", "John says hi. What can I help you with?");
        return;
    }
    ...
    if (message.toLowerCase().includesOneOf(["help", "support", "service"])) {
        pushMessage(
            "left",
            "John says our support is ready to help you on our phone customer service!<br>This isn't a task for John..."
        );
        return;
    }

    if (message.toLowerCase().includesOneOf(["meow", "cat"])) {
        pushMessage(
            "left",
            "*John makes cat sounds*"
        );
        return;
    }

    if (message.toLowerCase().includesOneOf(["live", "from", "country"])) {
        pushMessage(
            "left",
            "John lives in his mom's basement in Madhya Pradesh, India!<br>Benjamin and Oliver pay John's bills"
        );
        return;
    }
    ...
    if (message.toLowerCase().includesOneOf(["oliver", "benjamin"])) {
        pushMessage(
            "left",
            "John loves Oliver and Benjamin because those are his creators <3"
        );
        return;
    }
